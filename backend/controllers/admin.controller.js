...
exports.summary = asyncHandler(async (req, res) => {
  const [legacy, cases] = await Promise.all([
    adminService.analyticsSummary(),
    caseService.adminSummary()
  ])
  return success(res, { legacy, cases })
})

// ----- Case management (manual upload flow) -----
exports.listCases = asyncHandler(async (req, res) => {
  const data = await caseService.adminList({
...
exports.getCase = asyncHandler(async (req, res) => {
  const doc = await caseService.adminGetById(req.params.id)
  if (!doc.notifiedAdmin) {
    doc.notifiedAdmin = true
    await doc.save()
  }
  return success(res, { case: doc })
})

exports.updateCase = asyncHandler(async (req, res) => {
  const doc = await caseService.adminUpdate({
    id: req.params.id,
    patch: req.body,
    actor: req.user
  })
  return success(res, { case: doc }, 'Case updated')
})
