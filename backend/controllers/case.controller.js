...
// ---- Payment (initiated from the track page once a price is quoted) ----
...
exports.createPayment = asyncHandler(async (req, res) => {
  ...
  const payment = await Payment.create({
    user: doc.user || null,
    resolutionRequest: doc._id, // reuse field as polymorphic link (Case)
    amount: amountPaise,
    currency: 'INR',
    orderId: order.id,
    status: 'created'
...
exports.verifyPayment = asyncHandler(async (req, res) => {
  ...
  const updated = await caseService.markPaid({ id: req.params.id, payment })
